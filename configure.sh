...
if [ `command -v edep-sim` ]; then
    ...
    export LARCV_EDEPSIM_LIBS="${LARCV_EDEPSIM_LIBS} -L${where}/lib -ledepsim -ledepsim_io "
	  export ROOT_INCLUDE_PATH="$ROOT_INCLUDE_PATH:$where/include"
else
...
if [ $LARCV_NUMPY -eq 0 ]; then
    ...
    numpy_include="`$LARCV_PYTHON -c\"import numpy; print(numpy.get_include())\"`"
    LARCV_INCLUDES="${LARCV_INCLUDES} -I$numpy_include"
    LARCV_LIBS="-L`${LARCV_PYTHON_CONFIG} --prefix`/lib/ `${LARCV_PYTHON_CONFIG} --ldflags` ${LARCV_LIBS}"
    export ROOT_INCLUDE_PATH="$ROOT_INCLUDE_PATH:$numpy_include"
fi
if [[ $missing ]]; then
